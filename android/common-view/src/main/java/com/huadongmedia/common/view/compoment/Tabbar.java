...
public class Tabbar extends LinearLayout {
	...
	public Tabbar(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
		...
		TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.Common_Tabbar, defStyleAttr, R.style.Common_Tabbar);
		...
		try {
			...
			selectedTextColor = typedArray.getColor(R.styleable.Common_Tabbar_tabbarSelectedTextColor, Color.parseColor("#e63528"));
			Drawable dividerBackgroundDrawable = typedArray.getDrawable(R.styleable.Common_Tabbar_tabbarDividerBackground);
	...
	public void refresh(int index) {
		...
		if (itemLinearLayout != null) {
			((ImageView) itemLinearLayout.findViewById(R.id.icon_image_view))
					.setImageDrawable(index == selectedIndex ? adapter.getSelectedIcon(index) : adapter.getNormalIcon(index));
	...
	private void initItems() {
		...
		for (int i = 0; i < adapter.getCount(); ++i) {
			final LinearLayout itemLinearLayout = (LinearLayout) layoutInflater.inflate(R.layout.tabbar_action_item, itemPanelLayout, false);
